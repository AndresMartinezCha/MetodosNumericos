...
ui <- fluidPage(
  h2('Obtiene una raíz de un polinomio con el método de Falsa Posición'),
...
server <- function(input, output, session) {
  ...
  raiz <- eventReactive(input$buscar,{
    ...
    a <- input$inf
    b <- input$sup
    if(f(a)*f(b)>0) return(list('El intervalo es incorrecto', NULL))
    error <- input$error
    n_max <- input$n_max
    ...
    while (i<n_max&!encontrado) {
      ...
      if(fx1==0) valor <- a
      if(fx2==0) valor <- b
      xm <- b-(fx2*(a-b))/(fx1-fx2)
      merror <- abs((xm-a)/xm)
      if(merror<error) valor <- xm
      if(f(xm)<0&f(b)>0) a <- xm else b <- xm
      encontrado <- valor==xm
      i <- i+1
      tabla[i,1] <- a
      tabla[i,2] <- f(a)
    ...
    return(list(paste('El valor de la raiz es', round(valor,2)), tabla))
  })
  ...
  output$grafica <- renderPlot({
    ...
    isolate({minf <- input$inf})
    isolate({msup <- input$sup})
    xs <- seq(minf-1,msup+1,0.01)
    ys <- f(xs)
